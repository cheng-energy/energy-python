...
```py
# 排序，升序和降序，需要指定某一列
df.sort_values('列名',ascending=False)----ascending是上升的意思，如果为True是升序
# 还可以多列排序，因为一列有的数据可能一样
df.sort_values(['列名1','列名2'],ascending=[False,False])
```
数据的分组
```py
df.groupby('类名')['销售额'].sum()-----分组后求和
df.groupby(['类名1','类名2'])['销售额'].sum()-----分组后求和-----['销售额']是指定销售额那一列
df.groupby('类名')['销售额'].agg(['sum','max','min'])-----分组后求和,求最大值最小值
```
`分组后必须跟聚合函数：sum()、count()、mean()、max() 等，不然不会有计算结果`
